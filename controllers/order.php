...
class Order extends IController
{
	...
	// 获取全部店铺ID
	function get_all_shop($seller_id) {
		$shop = new Seller(IWeb::$app,'Seller');
		return '('.join(',', $shop->get_all_shop($seller_id)).')';
	}
	...
	public function order_refundment_doc()
	{
		...
		//1,用户已经提交退款申请的情况,获取关联orderGoodsID
		if($refunds_id)
		{
			$refundmentRow  = $tb_refundment_doc->getObj('id = '.$refunds_id);
			$order_goods_id = explode(",",$refundmentRow['order_goods_id']);
		}

		$orderGoodsIds  = join(",",$order_goods_id);
		$orderGoodsList = $orderGoodsDB->query('id in ('.$orderGoodsIds.') and is_send != 2');
		if(count($orderGoodsList) != count($order_goods_id))
		{
			die('<script text="text/javascript">parent.actionCallback("订单中没有符合退货条件的商品");</script>');
		}
    ...
    public function order_list()
    {
		...
		$orderHandle->order  = "o.id " . $sorting;
		...
		$search['sorting'] = $sorting == 'desc' ? 1 : 2;
		$this->search      = $search;
	...
	public function shop_template()
	{
		...
		$tb_order = new IModel('order');
		$where    = $seller_id ? 'id='.$order_id.' and seller_id in '.$this->get_all_shop($seller_id) : 'id='.$order_id;
		...
		if($data['seller_id'])
		{
			...
			$config_info = $sellerObj->getObj('id = '.$data['seller_id']);
	     	...
	     	$data['set']['name']   = isset($config_info['shopname'])? $config_info['shopname'] : '';
	...
	{
		$this->layout='print';
		$order_id = (array) (IFilter::act( IReq::get('id'),'int' ));
		$seller_id= IFilter::act( IReq::get('seller_id'),'int' );

		$order_list = ' in (' . join(',', $order_id) . ')';

		$tb_order = new IModel('order');
		$where    = $seller_id ? 'id'.$order_list.' and seller_id in '.$this->get_all_shop($seller_id) : 'id'.$order_list;
		$order_info     = $tb_order->query($where);
		...
		foreach ($order_info as $key => &$data) {
			if(!$data)
			{
	 		...
	 		$data['address'] = $data['addregion'] . ' ' . $data['address'];
		}

		// $this->setRenderData($data);
		$this->order_info = $order_info;
		$this->redirect('pick_template', false);
	}
	//合并购物单和发货单
	public function merge_template()
	{
		...
		//合并购物单和发货单
	public function merge_template2()
	{
		$this->merge = 1;
		$this->merge_template();
		$this->pick_template();
	...
	// 收款单导出
	public function order_collection_report() {
		$search = IReq::get('search') ? IFilter::act(IReq::get('search'),'strict') : array();
		$sorting = IFilter::act(IReq::get('sorting'),'strict');
		$order = 'desc';
		if (isset($sorting)) {
			if ($sorting == 1) {
				$order = 'desc';
		...
		$orderHandle = new IQuery('collection_doc as c');
		$orderHandle->order  = "c.time $order,o.id desc";
		$orderHandle->fields = "o.order_no,c.amount,u.username,p.name,c.id,c.pay_status,c.time";
		$orderHandle->join   = "left join order as o on c.order_id = o.id left join user as u on u.id = c.user_id left join payment as p on c.payment_id = p.id";
		$orderHandle->where  = "$where and c.if_del = 0";
		...
		$strTable .= '<tr>';
		$strTable .= '<td style="text-align:center;font-size:12px;">序号</td>';
		$strTable .= '<td style="text-align:center;font-size:12px;width:120px;">订单编号</td>';
		$strTable .= '<td style="text-align:center;font-size:12px;" width="100">收款金额</td>';
		$strTable .= '<td style="text-align:center;font-size:12px;" width="*">付款人</td>';
		...
		foreach($orderList as $k=>$val){
			...
			$strTable .= '<td style="text-align:left;font-size:12px;">'.($val['pay_status']==1?'支付完成':'准备中').' </td>';
			$strTable .= '<td style="text-align:left;font-size:12px;">'.$val['time'].' </td>';
			$strTable .= '</tr>';
		}
		$k++;
		if ($sorting == 1) {
				$strTable .="<tr><td colspan='7'>合计：{$to}元 $k 条记录 <br/>时间：{$orderList[count($orderList) - 1]['time']} ~ {$orderList[0]['time']} </td></tr>";
			} else {
				$strTable .="<tr><td colspan='7'>合计：{$to}元 $k 条记录 <br/>时间：{$orderList[0]['time']} ~ {$orderList[count($orderList) - 1]['time']}</td></tr>";
			}
		$strTable .='</table>';
		unset($orderList);
		$reportObj = new report();
		$reportObj->setFileName('收款单导出');
		$reportObj->toDownload($strTable);
		exit();
	}

	// 退款单导出
	public function order_refundment_report() {
		...
		$orderHandle = new IQuery('refundment_doc as c');
		$orderHandle->order  = "c.time $order,c.id desc";
		$orderHandle->fields = "c.*,u.username";
		$orderHandle->join   = "left join user as u on u.id = c.user_id";
		$orderHandle->where  = "$where and c.if_del = 0 and c.pay_status != 0";
		$orderList = $orderHandle->find();
		...
		foreach($orderList as $k=>$val){
			...
			$strTable .= '<td style="text-align:center;font-size:12px;">'.$val['order_no'].'</td>';
			$strTable .= '<td style="text-align:left;font-size:12px;">'.$val['amount'].' </td>';
			$strTable .= '<td style="text-align:left;font-size:12px;">'.($val['username']=='' ? '游客' : $val['username']).' </td>';
			$strTable .= '<td style="text-align:left;font-size:12px;">'.Order_Class::refundmentText($val['pay_status']).' </td>';
			$strTable .= '<td style="text-align:left;font-size:12px;">'.$val['dispose_time'].' </td>';
			$strTable .= '</tr>';
		}
		$k++;
		if ($sorting == 1) {
				$strTable .="<tr><td colspan='6'>合计：{$to}元 $k 条记录 <br/>时间：{$orderList[count($orderList) - 1]['time']} ~ {$orderList[0]['time']} </td></tr>";
			} else {
				$strTable .="<tr><td colspan='6'>合计：{$to}元 $k 条记录 <br/>时间：{$orderList[0]['time']} ~ {$orderList[count($orderList) - 1]['time']}</td></tr>";
			}
		$strTable .='</table>';
		unset($orderList);
		$reportObj = new report();
		$reportObj->setFileName('退款单导出');
		$reportObj->toDownload($strTable);
		exit();
	}
	...
	function note_edit_update() {
		$note = IFilter::act(IReq::get('note'),'text');
		$postscript = IFilter::act(IReq::get('postscript'),'text');
		$order_id = IFilter::act(IReq::get('order_id'),'int');
		$updateData = array('note' => $note, 'postscript' => $postscript);
		$goodsDB = new IModel('order');
		$goodsDB->setData($updateData);
		$goodsDB->update('id = '.$order_id);
		die(JSON::encode(array('result' => 'success','data' => $updateData)));
	}


	function order_print() {
		$name = IFilter::act(IReq::get('name'),'text');
		$merge = IFilter::act(IReq::get('merge'),'int');
		$order_id = (array) IFilter::act(IReq::get('order_id'),'int');
		$order_list = ' in (' . join(',', $order_id) . ')';
		if ($merge) {
			$updateData = array($name => 1);
		} else {
			$updateData = array('merge_print_state' => 1, 'pick_print_state' => 1);
		}
		$goodsDB = new IModel('order');
		$goodsDB->setData($updateData);
		$goodsDB->update('id'.$order_list);
