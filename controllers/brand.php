...
class Brand extends IController
{
	...
	function best_edit()
	{
		...
		if($brand_id)
		{
			$obj_brand = new IModel('commend_goods_category');
			...
			{
				$this->redirect('best_list');
				Util::showMessage("没有找到相关标签选项！");
				return;
			}
		}

		$this->setRenderData($this->data);
		$this->redirect('best_edit',false);
	}
	/**
	 * @brief 保存标签
	 */	
	function best_save()
	{
		...
		if (isset($_FILES['banner']['name']) && $_FILES['banner']['name']!='') {
			$uploadObj = new PhotoUpload('','brand');
	...
	function brand_save()
	{
		...
		{	
			...
			$brand_district_extend = new IModel('brand_district_extend');

			foreach ($district as $value) {
				$brand_district_extend->setData(array('brand_id' => $brand_id, 'district_id' => $value));
				$brand_district_extend->add();
			}

			// $districts = join(',',$district);
			// $brand['district_id'] = $districts;
		}
		else
		{
			// $brand['district_id'] = '';
