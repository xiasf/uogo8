...
class Site extends IController
{
	...
	function pro_list()
	{
		...
		{		//获取全部子分类
				$this->childId = goods_class::catChildAll();
		}
	...
	function products_info() {
		...
		$this->redirect('products_info');
	}
	//手机版详情
	function products_info_m() {
		$goods_id = IFilter::act(IReq::get('id'),'int');
		if(!$goods_id)
		{
			IError::show(403,"传递的参数不正确");
	...
	function products()
	{
		...
		// echo $goods_info['recommended_url'];
		if (!empty($goods_info['recommended_url'])) {
			header('Location: ' . $goods_info['recommended_url']);
		}
		$this->config["title"] = $goods_info["name"]." - ";//改变标题
		//品牌名称
		if($goods_info['brand_id'])
		{
			$tb_brand = new IModel('brand');
		...
		foreach($goods_info['photo'] as $key => $val)
		{
			...
			if($val['img'] == $goods_info['img'])
			{
				...
				// 优化改进后的 update：2016-3-7 15:23:55
				$temp = $goods_info['photo'][$key];
				unset($goods_info['photo'][$key]);
				array_unshift($goods_info['photo'], $temp);
			}
		}
	...
	//商品团购展示
	function products_tuan()
	{
		$goods_id = IFilter::act(IReq::get('id'),'int');

		if(!$goods_id)
		{
			IError::show(403,"团购传递的参数不正确");
			exit;
		}
		...
		$this->setRenderData($goods_info);
		$this->redirect('products_tuan');
	}
	//商品抢购展示
	function products_time()
	{
		...
		$order_model = new IQuery('order as o');
		$order_model->join = 'left join order_goods as og on og.order_id = o.id';
		$order_model->where = 'goods_id = ' . $goods_id . ' and o.user_id = ' . $uid;
		$this->is_buyed = false;
		if ($order_model->find()) {
			$this->is_buyed = true;
		}
	...
	function comment_ajax()
	{
		...
		$commentDB->order  = 'c.comment_time desc';
		$commentDB->page   = $page;
		$data     = $commentDB->find();
	...
	function comments_list()
	{
		...
		$query->order    = "a.comment_time DESC";
	...
	public function comment_add()
	{
		...
		$data['comment_time'] = date("Y-m-d H:i:s",ITime::getNow());
	...
	function activity_detail()
	{
		...
		$tb_activity = new IModel("activity");
		$act_row = $tb_activity->query("id={$id}");
		if($act_row)
		{
			$this->act_row = end($act_row);
		}
		else
		{
			IError::show(404,"您查找的活动已经不存在了");
		}
		$this->redirect("activity_detail");
	}
