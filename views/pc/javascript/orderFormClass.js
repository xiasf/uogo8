...
function orderFormClass()
{
	...
	this.addressSelected = function(jsonData)
	{
		...
		jsonData.addregion = jsonData.addregion ? jsonData.addregion:"湖北省/黄冈市/黄州区";
	...
	this.addressSave = function()
	{
		...
		if(!this.addressActiveId)
		{
			...
			var jsonData =
			{
				"addregion":$('input[name="addregion"]').val(),
		...
		if (1) {
			if ($('input[name=addregion]').val().indexOf('黄州') != -1) {
				if ($('input:radio[name="delivery_id"]:checked').val() != 3) {
					$('#deliveryToggleButton').trigger('click');
					$('label[id$="_save_button"]:visible').first().trigger('focus');
					alert('为避免承担不必要的运费，黄州区的用户请选择系统默认配送方式哦。');
					return false;
				}
			}
		}
