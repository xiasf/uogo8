(function($) {
	...
	$.fn.jfocus = function(settings) {//首页焦点广告图切换
		...
		$.fn.jctab = function(settings) {//首页楼层定义切换
		var defaults = {
			time: 5000
		};
		var settings = $.extend(defaults, settings);
		return this.each(function(){
			var $this = $(this);
			var sWidth = $this.width();
			var len = $this.find(".floor-item").length;
			var index = 0;
			var picTimer;
			var btn = "<div class='pagination'>";
			for (var i = 0; i < len; i++) {
			...
			$this.find(".arrow").css("opacity", 0.3).hover(function() {
				$(this).stop(true, false).animate({
					"opacity": "0.8"
				},
				300);
			...
			$this.find(".floor-tabcnt").css("ed", sWidth * (len));
			$this.hover(function() {
				clearInterval(picTimer);
			},
			function() {
			...
			function showPics(index) {
				var nowLeft = -index * sWidth;
				$this.find(".floor-item").stop(true, false).animate({
					"left": -1200
				},300).eq(index).stop(true, false).animate({
					"left": 0
				},
				300);
				var nowLeftt = +index * 119;
				$this.find(".tab-slide").stop(true, false).animate({
					"left": nowLeftt
				},
				300);
				$this.find(".tab-item > li").removeClass("current").eq(index).addClass("current").stop(true, false).animate(
				300);
				$this.find(".pagination span").stop(true, false).animate({
					"opacity": "0.4"
				},
