...
var config = jQuery.extend( true, 
 {
	...
	defimg:"\/views\/sysseller\/img\/face_def.png",
    chating: {},
...
},win.layIM),dom = [$(window), $(document), $('html'), $('body')], xxim = {};
...
xxim.popchat = function(param){
    ...
    log.success = function(layero){
        ...
        log.chatlist.html('<li data-id="'+ param.id +'" type="'+ param.type +'"  id="layim_user'+ param.type + param.id +'"><em>×</em><span><img src="'+ param.face +'" onerror="this.src=\''+config.defimg+'\'">'+ param.username +'</span></li>')
        ...
        xxim.chatbox.find('.layer_setmin').on('click', function(){
            ...
            layero.hide();
            node.layimMin.text(xxim.nowchat.username).show();
        });
    ...
    log.html = '<div class="layim_chatbox" id="layim_chatbox" style="overflow:hidden">'
            +'<h6>'
            +'<span class="layim_move"></span>'
            +'    <p class="layim_face"><img src="'+ param.face +'" onerror="this.src=\''+config.defimg+'\'"></p>'
            +'    <p class="layim_username layim_names">username</p>'
            +'    <p class="layim_shopname layim_names">shopname</p>'
            +'    <p class="layim_goodname layim_names">goodname</p>'
            ...
            +'        <a href="javascript:;"><i class="layim_addimage fa fa-picture-o" title="发送图片"></i></a>'
            +'        <a href="javascript:;"><i class="layim_addfile fa fa-paperclip" title="发送附件"></i></a>'
    ...
    if(config.chatings < 1){
        ...
        log.chatmore.find('ul').append('<li data-id="'+ param.id +'" type="'+ param.type +'" id="layim_user'+ param.type + param.id +'" class="layim_chatnow"><em>×</em><span><img src="'+ param.face +'" onerror="this.src=\''+config.defimg+'\'">'+ param.username +'</span></li>');
...
xxim.tabchat = function(param){
    ...
    xxim.chatbox.find('.layim_username').text(param.username); 
    xxim.chatbox.find('.layim_shopname').html("<a href='/shop-"+param.shopid+".html' target=new><img src='"+param.shoplogo+"' onerror='this.src=\""+config.defimg+"\"'>" + param.shopname + "</a>"); 
    xxim.chatbox.find('.layim_goodname').html("<a href='/item-"+param.goodid+".html' target=new><img src='"+param.goodimg+"' onerror='this.src=\""+config.defimg+"\"'>" + param.goodname + "</a>"); 
...
xxim.popchatbox = function(othis){
    var node = xxim.node, dataId = othis.attr('data-id'),dataParam = $(othis).data("json") , param = $.extend({
        id: dataId, //用户ID
        type: othis.attr('type')
    },dataParam), key = param.type + dataId;
	//alert(dataParam.goodname)
...
/*xxim.getGroups = function(param){
    ...
    config.json(config.api.groups, {}, function(datas){
        if(datas.status === 1){
            ...
            if(lens > 0){
                for(; ii < lens; ii++){
                    str += '<li data-id="'+ datas.data[ii].id +'" type="one"><img src="'+ datas.data[ii].face +'" class="xxim_oneface"><span class="xxim_onename">'+ datas.data[ii].username +'</span></li>';
                }
            } else {
    ...
    log.send = function(){
        ...
        if(data.content.replace(/\s/g, '') === ''){
                        ...
                        + function(){
                            if(type === 'me'){
                                return '<span class="layim_chattime">'+ param.time +'</span>'
                                       ...
                                       +'<img src="'+ param.face +'" onerror="this.src=\''+config.defimg+'\'" >';
                            } else {
                                return '<img src="'+ param.face +'" onerror="this.src=\''+config.defimg+'\'" >'
                                       +'<span class="layim_chatname">'+ param.name +'</span>'
            ...
            log.imarea.append(log.html({
                ...
                name: "商户",
                face: "/views/sysseller/img/face_def.png",
...
//建立连接
xxim.websocket = function(options) {
        var defaults = {
        ...
        var szServer = "ws://" + opts.domain + ":"+opts.port+"/" + opts.protocol;//console.log(szServer);
        var socket = null;
        var bOpen = false;
        var t1 = 0,t2 = 0;
        var messageevent = {
...
xxim.getDates = function(index){
    var api = [],//config.api.friend, config.api.chatlog
        node = xxim.node, myf = node.list.eq(index),li,liul;
    myf.addClass('loading');
	config.websocket = xxim.websocket({
		domain:"114.215.243.68",   //这是与服务器的域名或IP
		//port:80,                  //这是服务器端口号
		...
		onError:function(event){
			myf.html('<li class="xxim_errormsg">与服务器建立连接方式错误<br>本功能不支持 IE 10 以下浏览器</li>');
    ...
    config.json(api[index], {}, function(datas){
        if(datas.status === 1){
            ...
            if(myflen > 0){
                ...
                if(index !== 1){//不是聊天记录
                    for(; i < myflen; i++){
						li = $('<li data-id="'+ datas.data[i].id +'" class="xxim_parentnode"></li>').appendTo(myf);
						li.append('<h5><i class="fa fa-caret-right"></i><span class="xxim_parentname">'+ datas.data[i].name +'</span><em class="xxim_nums">（'+ datas.data[i].nums +'）</em></h5>');
						liul = $('<ul class="xxim_chatlist"></ul>').appendTo(li);
                        item = datas.data[i].item;
                        for(var j = 0; j < item.length; j++){
							item[j].username == null && (item[j].username = "游客");
							item[j].face == null && (item[j].face = "");
                            $('<li data-id="'+ item[j].id +'" class="xxim_childnode '+(item[j].status == 0?"blink":"")+'" type="'+ (index === 0 ? 'one' : 'group') +'"><img src="'+ item[j].face +'" class="xxim_oneface" onerror="this.src=\''+config.defimg+'\'"><span class="xxim_onename">'+ item[j].username +'</span></li>').appendTo(liul).data("json",item[j]);
                        } 
                    }
                } else {
...
xxim.view = (function(){
    var xximNode = xxim.layimNode = $('<div id="xximmm" class="xxim_main">'
			...
			+'        <span class="xxim_tabfriend" title="客户"><i class="fa fa-user"></i></span>'
/*			+'        <span class="xxim_tabgroup" title="群组"><i class="fa fa-users"></i></span>'*/
			+'        <span class="xxim_latechat"  title="最近联系人"><i class="fa fa-clock-o"></i></span>'
			+'  </div>'
