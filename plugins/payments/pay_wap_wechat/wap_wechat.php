...
class wap_wechat extends paymentPlugin
{
	...
	public function getSendData($payment)
	{
		...
		$return['openid']           = wechat_facade::getOpenId();
		// print_r($_SESSION);
		// exit($return['openid']);
		...
		if(is_array($resultArray))
		{
			...
			if(isset($resultArray['return_code']) && $resultArray['return_code'] == 'SUCCESS')
			{
				...
				$resultArray['order_id'] = $payment['M_OrderId'];	// 微信移动支付参数返回加一个订单ID 夏凯 2016-3-17 15:08:33
				return $resultArray;
			}
			else
			{
	...
	public function doPay($sendData)
	{
		if(isset($sendData['prepay_id']) && $sendData['prepay_id'])
		{
			...
			// $return['successUrl'] = IUrl::getHost().IUrl::creatUrl('/site/success/message/'.urlencode('支付成功！').'/order_id/'.$sendData['order_id']);
			$return['failUrl'] = $return['successUrl'] = IUrl::getHost().IUrl::creatUrl('/ucenter/order_detail/id/'.$sendData['order_id']);
