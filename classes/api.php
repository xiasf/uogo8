...

{
...

	{
		...
		{
			//参数第一位 永远是限制数量
			if(is_numeric($aguments[0]))
			{
				$queryInfo['limit'] = $aguments[0];
			}
			//第二位是参数
			if(is_array($aguments[1])){
				
				$paramArray = [];
				foreach($aguments[1] as $index => $item){
					if($index%2==0)$paramArray[$item] = $aguments[1][$index+1];
					//$queryInfo['where'] = str_replace($paramArray,$queryInfo['where']);
				}
				//var_dump($paramArray);
				$queryInfo['where'] = strtr($queryInfo['where'],$paramArray);
				//echo $queryInfo['where'];	
			}else{
