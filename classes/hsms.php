...
class Hsms
{
	...
	public static function send($mobiles, $content, $delay = 1)
	{
		...
		if($delay == 1)
		{
			$smsTime = ISession::get('smsDelay');
			if($smsTime && time() - $smsTime < 59)
			{
		...
		if(self::$smsInstance == null)
		{
			...
			switch($platform)
			{
				...
				case "yunpian":
				{
					$classFile = IWeb::$app->getBasePath().'plugins/hsms/yunpian.php';
					require($classFile);
