...

{
...

	{
		...
		$configArray = new Config("site_config");

		$this->config = $configArray->getInfo();

		$this->config["url"] = IUrl::getHost();

		$this->config["callback"] = IReq::get('callback') ? urlencode(IFilter::act(IReq::get('callback'),'url')) : '';
		$this->config["model"] = $this->getId();
		$this->config["action"] = $actionId;

		$this->config["iswechat"] = IClient::isWechat();
		$this->config["cururl"] = urlencode(IUrl::getUrl());
		

		//$this->config["userkey"] = time();

		

        //var_dump($this->config);

		//echo $this->config["iswechat"];
