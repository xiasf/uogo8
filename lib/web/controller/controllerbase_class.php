	...
	public function renderView($viewFile,$rdata=null,$return=false)
	{
		...
		if(is_file($renderFile))
		{
			...
			if(stripos($renderFile,IWEB_PATH.'web/view/')===false)
			{
				...
				if($this->isRanderCache == false || !is_file($runtimeFile) || (filemtime($renderFile) > filemtime($runtimeFile)) || (is_file($layoutFile) && (filemtime($layoutFile) > filemtime($runtimeFile))))
				{
					//获取view内容
